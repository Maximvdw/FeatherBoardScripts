...
var ScrambleEffect = {
	...
	scramble: function(str){
		...
		var shiftedString = this.utils.shuffle(newString);
		print(JSON.stringify(newString));
		// Shuffle the array/string
		// Next, use the shuffled array to hide the characters one by one
		for (var i = 0 ; i < str.length ; i++){
			newString[shiftedString[i].idx].character = "&k" + newString[shiftedString[i].idx].character;
			var flatString = [];
			for (var j = 0 ; j < str.length ; j++){
				flatString.push(newString[j].character);
			}
			animation.push(flatString.join(""));
		}
		return animation;
